...
//#   define STREAM_ARRAY_SIZE	660000000
#   define STREAM_ARRAY_SIZE	360000000
#endif
...
main()
    ...
    //int ret = set_mempolicy(MPOL_PREFERRED, NULL, 0);
...
#pragma omp parallel for schedule(static)
    ...
    fprintf(stderr, "Initializing data done... running first test\n");

    int n_tasks_overall = -1;
#pragma omp parallel
#pragma omp master
    n_tasks_overall = omp_get_num_threads()*4;
...
{
    int step = STREAM_ARRAY_SIZE / n_tasks_overall;
    fprintf(stderr, "STREAM_ARRAY_SIZE = %d; n_tasks_overall = %d; step = %d\n", STREAM_ARRAY_SIZE, n_tasks_overall, step);
    if(STREAM_ARRAY_SIZE % n_tasks_overall != 0)
      step++;
    for(ntask = 0; ntask < n_tasks_overall; ntask++)
    {
      int tmp_idx_start = ntask * step;
      int tmp_idx_end = MIN((ntask+1)*step -1,STREAM_ARRAY_SIZE);
#ifdef TASK_AFFINITY
...
#endif
      #pragma omp task firstprivate(tmp_idx_start, tmp_idx_end) untied
      {
		    int i;
        for(i = tmp_idx_start; i <= tmp_idx_end; i++){
          c[i] = a[i];

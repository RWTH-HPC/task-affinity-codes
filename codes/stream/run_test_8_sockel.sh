...
function set_up_affinity {
    ...
    #NAME=STREAM_${TASK_AFF_NUMBER_OF_AFFINITIES}-${page_selection_strategy[$3+1]}-${page_weight_strategy[$4+1]}-${map_mode[$2+1]}${5}
...
#module load omp/task-affinity
#module li

compile ".affinity"
#for affinities in {2,5,10,15,20}
for affinities in {2,5}
do
    export TASK_AFF_NUMBER_OF_AFFINITIES=${affinities}
    echo "Number of affinities:\t\t ${affinities}"
    for page_mode in {0,1,3} # first_page_of_first_affinity, devide-in-n, first-and-last
    do
        for page_weight in {1,2}  # majority, by_affinity
        do
            set_up_affinity 2 0 ${page_mode} ${page_weight} ${OMP_NUM_THREADS} #thread
            # for i in {0..4}
            # do
            run ".affinity" ${i}
            # done
            ...
            set_up_affinity 2 1 ${page_mode} ${page_weight} ${OMP_NUM_THREADS} #domain
            # for i in {0..4}
            # do
            run ".affinity" ${i}
            # done

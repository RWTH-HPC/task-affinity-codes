...
static void convert_to_blocks_for_onesided(const int ts, const int DIM, const int N, double Alin[N][N], double *A)
{
	for (int i = 0; i < DIM; i++)
		for (int j = 0; j < DIM; j++) {
			gather_block ( N, ts, &Alin[i*ts][j*ts], A+(i*DIM+j)*ts*ts);
		}
}
